...
if ($role === 'admin') {
    ...
    $stmt->execute();
    $total_users = $stmt->get_result()->fetch_assoc()['c'];
    $stmt->close();
}
?>
<!DOCTYPE html>
...
<div class="container">
    <div class="dashboard-wrapper">
        ...
        <div class="topbar">
            <h1>🎓 EduTrack</h1>
            <span>Welcome, <strong style="color:white;"><?= htmlspecialchars($username) ?></strong> (<?= $role ?>) &nbsp;|&nbsp;
                <a href="logout.php">Logout</a></span>
        </div>

        <div class="stats-row">
            <div class="stat-card"><div class="stat-label">Total Students</div><div class="stat-value"><?= $total_students ?></div></div>
            <div class="stat-card green"><div class="stat-label">Year 1 & 2</div><div class="stat-value"><?= $year_counts[1] + $year_counts[2] ?></div></div>
            <div class="stat-card orange"><div class="stat-label">Year 3 & 4</div><div class="stat-value"><?= $year_counts[3] + $year_counts[4] ?></div></div>
            <div class="stat-card purple"><div class="stat-label">Library Books</div><div class="stat-value"><?= $total_books ?></div></div>
            <?php if ($role === 'admin'): ?>
            <div class="stat-card blue"><div class="stat-label">Registered Users</div><div class="stat-value"><?= $total_users ?></div></div>
            <?php endif; ?>
        </div>
